...
def run_epoch(model,val_loader,use_age=False):
    ...
    return logit_all_catted.numpy()


def evaluation_models(model_name,data_loader, expansion_list = [8],  num_trails = 10,percentage = .01, use_age = False, norm_type= 'Instance'):
    ...
    for i,ep in enumerate(expansion_list):
        ...
        model = model.eval()
        logit_all = run_epoch(model, data_loader, use_age)
...
if __name__ == '__main__':
    ...
    model_file_name = 'volume_finetune_flair/volume_finetune_flair_train_perc_100.0_expansion_8'
    ...
    ans = {'res': res, 'out': out}
    with open('res_flair_finetune.pkl', 'wb') as output_file:
        pickle.dump(ans, output_file)
